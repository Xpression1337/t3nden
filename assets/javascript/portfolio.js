...
$(document).ready(() => {
  const links = [
    {
      name: 'T3nDen#5701',
      link: 'https://discord.com/users/255005880863424522',
    },
    {
      name: 'Steam',
      link: 'https://steamcommunity.com/id/T3nDen',
    },
    {
      name: 'Twitch',
      link: 'https://www.twitch.tv/t3nden_',
    },
  ];
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authentifiziere...', "Zugriff auf <span style='font-size: 14px; color: #06d;'>[T3nDen.cf]</span>..."], 30, () => {
    ...
    writeLine([`Zugriff gewährt! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Willkommen, <i style='color: #0f0'>${ip}</i>! Schön jemanden aus ${country} zu sehen!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #ffc0cb'>made by xpression $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
